...
namespace Rent_A_Car.MVC.Areas.Admin.Controllers
{
    ...
    public class BrandController(IBrandService _service) : Controller
    {
        ...
        public async Task<IActionResult> Create(BrandCreateDTO dto)
        {
            ...
            {
                ModelState.AddModelError("", "Brand has been exists.");
                return View();
            }
        ...
        public async Task<IActionResult> Update(BrandUpdateDTO dto, int? id)
        {
            if (!id.HasValue)
                return BadRequest();
            if (!ModelState.IsValid)
            ...
            if (!await _service.GetExistsAsync(dto))
            {
                ModelState.AddModelError("", "Brand has been exists or not changed(If you don't wanna change, go back).");
                return View();
            }
